...
func downloadFromAmazon(req DownloadRequest) (AmazonDownloadResult, error) {
	...
	if req.EmbedLyrics {
		...
		if parallelResult != nil && parallelResult.LyricsLRC != "" {
			...
			GoLog("[Amazon] Using lyrics from parallel fetch (%s, %d lines)\n", source, len(parallelResult.LyricsData.Lines))
		} else {
			GoLog("[Amazon] No lyrics from initial fetch, trying LRCLIB fallback...\n")
			...
			if err == nil && lrcData != nil {
				...
				source = "lrclib"
				GoLog("[Amazon] Found lyrics on LRCLIB (%s)\n", lrcData.Source)
			} else {
				GoLog("[Amazon] No lyrics found on LRCLIB.\n")
			}
		}

		if lyricsToEmbed != "" {
			if embedErr := EmbedLyrics(outputPath, lyricsToEmbed); embedErr != nil {
				GoLog("[Amazon] Warning: failed to embed lyrics from %s: %v\n", source, embedErr)
			} else {
				GoLog("[Amazon] Lyrics from %s embedded successfully\n", source)
			}
		}
	}
